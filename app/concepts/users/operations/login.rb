...
class User::Login < Trailblazer::Operation
  ...
  def check_email_existence(_options, params:, **)
    @user = User[email: params[:user][:email]]
  end

  def invalid_email!(options, params:, **)
    options[:errors] = {
      message: "Account with email: '#{params[:user][:email]}' does not exists.",
      status: 404
    }
  end
  ...
  def add_refresh_token_to_db(_options, params:, **)
    refresh_token = RefreshToken[device_id: params[:user][:device_id],
                                 user_id: @user.id]
    if refresh_token
