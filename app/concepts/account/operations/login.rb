...
module Account
  class Login < Trailblazer::Operation
    ...
    step Wrap(SequelTransaction) {
      ...
      step :user_active?
      failure :user_deactivated!, fail_fast: true

      step :user_confirmed?
      failure :user_not_confirmed!, fail_fast: true
    ...
    def invalid_email!(options, params:, **)
      options[:errors] = {
        message: "User with email: '#{params['email']}' does not exists.",
        status: 404
      }
    end
    ...
    def user_active?(_options, *)
      @user.active
    end

    def user_deactivated!(options, *)
      options[:errors] = {
        message: "User with email: '#{@user.email}' deactivated.",
        status: 401
      }
    end
    ...
    def user_not_confirmed!(options, *)
      options[:errors] = {
    ...
    def generate_access_token(_options, **)
      @access_token = Authentication::JsonWebToken.encode(
        { Authentication::JsonWebToken::USER_IDENTIFIER => @user.id },
        @access_time.to_i
      )
    end
    ...
    def add_refresh_token_to_db(_options, params:, **)
      refresh_token = ::RefreshToken[device_id: params['device_id'],
                                   user_id: @user.id]
      if refresh_token
